...
//uint16_t chek = 0;
//uint16_t err_chek = 0;
char tempModbusRx2[BUFFERSIZE];

/*Runs when data retrieval is complete and check CRC*/
bool uartDataHandler(void)
{
	uint8_t tempCounter;
	uint16_t CRCValue;
	...
	if(uartPacketComplatedFlag == SET)     //Data receiving is finished
	{
	  ...
	  tempCounter = DataCounter;
		...
		/*CRC Check*/		CRCValue = MODBUS_CRC16(tempModbusRx, tempCounter - 2);
...
{
	HAL_GPIO_WritePin(PC_UART2_EN_GPIO_Port, PC_UART2_EN_Pin, GPIO_PIN_SET); //поднимаем ножку 
  HAL_UART_Transmit_DMA(&huart2, (uint8_t *)msg, len);  //отправляем данные
	RxInterruptFlag = RESET;
	check2++;	
}
